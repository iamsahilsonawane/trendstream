...
import 'movie_item_v3.dart';

class MoviesV3Grid extends HookConsumerWidget {
  const MoviesV3Grid({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final popularMoviesCount = ref.watch(moviesV3Provider);

    return popularMoviesCount.when(
      data: (movies) {
        return AlignedGridView.count(
          key: const PageStorageKey<String>(
              'preserve_movies_v3_grid_scroll_and_focus'),
          ...
          itemBuilder: (BuildContext context, int index) {
            return ProviderScope(
              overrides: [
                currentMovieV3Provider
                    .overrideWithValue(AsyncValue.data(movies[index]))
              ],
              child: MovieTileV3(autofocus: index == 0),
            );
          },
        );
      },
